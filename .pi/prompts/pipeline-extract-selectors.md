...
1. Read `results/$1/$2/step2-discover/snapshot.merged.yaml` when present; otherwise read `results/$1/$2/step2-discover/snapshot.yaml`.
2. Read all per-path snapshots from `results/$1/$2/step2-discover/paths/` when present.
3. Read `results/$1/$2/step2-discover/selector-candidates.md`.
4. Read `results/$1/$2/step1-resolve/flow-inventory.json` when present to preserve selected flow and start-path provenance.
...
7. Write normalized selectors to `results/$1/$2/step3-extract-selectors/normalized-selectors.md`.
