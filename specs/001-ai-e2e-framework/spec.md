...
3. **Given** page objects are approved, **When** the AI agent drafts Gherkin `.feature` test scenarios and a scenario coverage mapping and presents them for review, **Then** the human can approve or request changes, and only approved scenarios proceed to spec generation.
4. **Given** test scenarios are approved, **When** the AI agent promotes the approved feature file and writes the Playwright spec to `tests/example/`, **Then** the spec compiles without TypeScript errors, uses approved page objects, and preserves traceability to each approved scenario or scenario outline example row.
...
- **FR-020**: The pipeline MUST enforce human review gates on two specific steps before their artifacts are promoted from `results/` to source directories: (a) the draft page object step (page object promoted to `src/pages/<app>/`), and (b) the draft tests step (approved Gherkin feature file promoted to `tests/<app>/<app>.feature`). These are the only gated steps in v1. Selector candidates and discovery snapshots are NOT gated — they remain in `results/` as reference artifacts only.
- **FR-021**: The human review mechanism MUST be a clear, chat-based workflow: (1) AI presents the full draft artifact text inline in the chat session for the human to read, (2) human runs `/pipeline-continue` to approve the current gate; the runner sends `approved` to the agent, promotes the artifact, and advances the pipeline, or the human provides change feedback as free-form text, (3) if changes are requested, AI re-drafts the artifact using the feedback as guidance and re-presents it. Maximum 3 re-draft attempts per artifact; if convergence is not reached, the step is marked as blocked with the unresolved feedback recorded.
...
- **SC-005**: Two different AI agents (starting with pi, architecture-ready for a second) can follow the same manifest workflow (via their respective adapters) and produce pipeline artifacts where: (a) the same required artifact files exist at the same relative paths, (b) selector candidates use the same locator priority order (role → testid → label → text → CSS), (c) generated tests cover the same set of interactive elements identified in the discovery snapshot, and (d) every generated Playwright test traces to exactly one approved Gherkin scenario or scenario outline example row.
