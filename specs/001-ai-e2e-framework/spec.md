...
- **FR-022**: Approved artifacts MUST NOT be silently overwritten by subsequent pipeline runs. If a subsequent run would overwrite an approved artifact, the AI MUST: (1) detect that the destination file already contains a provenance header from a previous approval, (2) present a diff of the proposed changes to the artifact to the human, (3) request explicit re-approval before writing. The human's re-approval triggers a provenance header update with the new run ID and timestamp.
