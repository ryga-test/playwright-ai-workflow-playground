...
- [X] T024 [US2] Create `adapters/pi/README.md` documenting: (a) how pi slash commands map to pipeline steps, (b) how to invoke each step (`/pipeline-resolve example`, `/pipeline-discover example <run>`, etc.), (c) how placeholders (`{{app}}`, `{{run}}`) are resolved, (d) the gated step flow (approve/re-draft), (e) how to add a second adapter at `adapters/<agent>/capabilities.yaml`.
- [X] T025 [US2] Verify manifest completeness: confirm 8 steps, confirm steps 4 and 5 have `gated: true`, confirm all paths use `{{app}}` and `{{run}}` placeholders consistently. Confirm adapter maps all 8 steps with no unmapped capabilities.
- [X] T026 [US2] Verify adapter extensibility: confirm `contracts/adapter.schema.yaml` requires only `adapter` name + `capabilities` array. Confirm a hypothetical `adapters/claude/capabilities.yaml` could be added without editing `workflows/manifest.yaml` (verifiable by reading manifest — it has no agent-specific references).
...
- [ ] T037 [US3] **Step 5 (Draft Tests) ⚠️ GATED**: Run `/pipeline-draft-tests example <run>`. Verify outputs: `results/example/<run>/step5-draft-tests/test-scenarios.feature` contains valid Gherkin scenarios and `results/example/<run>/step5-draft-tests/scenario-coverage.md` maps scenarios to discovered interactive elements. Verify the AI presents both for review and does NOT proceed to spec writing until approval. On approval: promote to `tests/example/example.feature` with provenance comments. On rejection: re-draft loop same as Step 4.
...
- [ ] T039 [US3] **Automatic Spec Overwrite**: Run `/pipeline-write-spec example <new-run>` a second time (simulating a subsequent pipeline run). Verify the AI detects the existing provenance header in `tests/example/example.spec.ts`, writes a diff artifact to `results/example/<new-run>/step6-write-spec/spec.diff`, overwrites `tests/example/example.spec.ts` automatically without requesting re-approval, and updates the provenance header with the new run ID.
